...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        choices=["fundamental", "valuation", "management", "full", "all"],
...
def show_banner():
    ...
    console.print()
    console.print("  Modes    : fundamental | valuation | management | full | all")
    console.print()
...
def run_generic_analysis(ticker: str, use_gpt: bool, mode: str,
    ...
    if mode == "all" and fund_result and val_result and mgmt_result:
        _print_combined_summary(ticker, fund_result, val_result, mgmt_result)

    return fund_result, val_result, mgmt_result
...
def run_banking_analysis(ticker: str, use_gpt: bool, mode: str,
    ...
    if mode == "all" and banking_result and mgmt_result:
        _print_banking_combined_summary(ticker, banking_result, mgmt_result)

    return banking_result, mgmt_result
...
def _print_combined_summary(ticker, fund, val, mgmt):
    ...
    t.add_row("[bold]⭐ OVERALL[/bold]",
              ...
              "[bold]Fund 40% + Val 30% + Mgmt 30%[/bold]")
    console.print(t)
    console.print()
...
def _print_banking_combined_summary(ticker, banking, mgmt):
    from rich.table import Table
    from rich import box
    console.print()
